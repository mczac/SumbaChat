...
3. Copy local deployment hosts:
   ...
   ```bash
   cp NextcloudTalk/Settings/NCAppBrandingLocal.example.h \
      NextcloudTalk/Settings/NCAppBrandingLocal.h
   ```
...
- SumbaChat branding (icon, login, display name)
- Photo/video compression before upload (Automatic / Choose on upload)
- Shared App Group media cache with Settings → Caching controls
- Dedicated push proxies (configured via `NCAppBrandingLocal.h`)
